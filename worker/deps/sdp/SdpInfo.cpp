...
static const char *SDP_IDENTIFIER = "WebRTC Gateway";
...
std::string SdpInfo::stringifyCandidate(const CandidateInfo &candidate) {
	...
	sdp << "a=candidate:" << candidate.foundation << " "
			<< candidate.componentId << " " << candidate.netProtocol << " "
			<< candidate.priority << " " << candidate.hostAddress << " "
			<< candidate.hostPort << " typ " << hostType_str;
...
bool SdpInfo::isValidExtension(std::string uri) {
	auto value = std::find_if(supported_ext_map_.begin(),
			supported_ext_map_.end(), [uri](const ExtMap &extension) {
				return extension.uri == uri;
			});
	return value != supported_ext_map_.end();
}

void SdpInfo::setCredentials(const std::string &username,
		const std::string &password, MediaType media) {
	switch (media) {
	case (VIDEO_TYPE):
	...
	if (printedAudio && this->hasAudio) {
		...
		for (uint8_t i = 0; i < extMapVector.size(); i++) {
			if (extMapVector[i].mediaType == AUDIO_TYPE
					&& isValidExtension(std::string(extMapVector[i].uri))) {
				sdp << "a=extmap:" << extMapVector[i].value << " "
						<< extMapVector[i].uri << endl;
			}
		}

		if (bundleTags.size() > 2) {
			ELOG_WARN(
					"More bundleTags than supported, expect unexpected behaviour");
		}
		for (uint8_t i = 0; i < bundleTags.size(); i++) {
		...
		for (unsigned int it = 0; it < payloadVector.size(); it++) {
			...
			if (rtp.media_type == AUDIO_TYPE) {
				...
				if (rtp.channels > 1) {
					sdp << "a=rtpmap:" << payload_type << " "
							<< rtp.encoding_name << "/" << rtp.clock_rate << "/"
							<< rtp.channels << endl;
				} else {
					sdp << "a=rtpmap:" << payload_type << " "
							<< rtp.encoding_name << "/" << rtp.clock_rate
				...
				if (!rtp.feedback_types.empty()) {
					for (unsigned int itFb = 0;
							itFb < rtp.feedback_types.size(); itFb++) {
						sdp << "a=rtcp-fb:" << payload_type << " "
								<< rtp.feedback_types[itFb] << "\n";
					}
				}
				if (!rtp.format_parameters.empty()) {
					std::string fmtp_line;
					for (std::map<std::string, std::string>::const_iterator theIt =
							rtp.format_parameters.begin();
					...
					fmtp_line.pop_back();
					sdp << "a=fmtp:" << payload_type << " " << fmtp_line
							<< std::endl;
				}
			}
		}
...
RtpMap* SdpInfo::getCodecByExternalPayloadType(
...
bool SdpInfo::supportCodecByName(const std::string codecName,
		const unsigned int clockRate) {
...
void SdpInfo::createOfferSdp(bool videoEnabled, bool audioEnabled,
		bool bundle) {
	ELOG_DEBUG("Creating offerSDP: video %d, audio %d, bundle %d", videoEnabled,
			audioEnabled, bundle);
	...
	while (std::getline(iss, line)) {
		...
		if (isGroup != std::string::npos) {
			...
			if (parts.size() >= 3) {
				for (unsigned int tagno = 2; tagno < parts.size(); tagno++) {
					ELOG_DEBUG("Adding %s to bundle vector",
							parts[tagno].c_str());
		...
		if (isCrypt != std::string::npos) {
			...
			ELOG_DEBUG("Crypto Info: %s %s %d", crypinfo.cipherSuite.c_str(),
					crypinfo.keyParams.c_str(), crypinfo.mediaType);
		}
		if (isUser != std::string::npos) {
			std::vector<std::string> parts = stringutil::splitOneOf(
					stringutil::splitOneOf(line, ":", 1)[1], "\r", 1);
			...
			if (mtype == VIDEO_TYPE) {
				...
				ELOG_DEBUG(
						"Unknown media type for ICE credentials, looks like Firefox");
				iceVideoUsername_ = parts[0];
			}
		}
			...
			if (parts.size() >= 2 && isBundle) {
				...
				for (uint8_t i = 0; i < bundleTags.size(); i++) {
					if (!bundleTags[i].id.compare(thisId)) {
						ELOG_DEBUG("Setting tag %s to mediaType %d",
								thisId.c_str(), mtype);
		...
		if (isRid != std::string::npos) {
			...
			if (mtype == VIDEO_TYPE) {
				...
				if (delimiter_pos != std::string::npos) {
					const auto direction = rid_raw.substr(delimiter_pos + 1,
							std::min(decltype(rid_raw.length()) { 4 },
									rid_raw.length() - delimiter_pos - 1));
					if (direction == "send") {
						rids_.push_back( { rid_raw.substr(0, delimiter_pos),
								RidDirection::SEND });
						ELOG_DEBUG("message: added simulcast rid send, id: %s",
								rids_.back().id.c_str());
					} else if (direction == "recv") {
						rids_.push_back( { rid_raw.substr(0, delimiter_pos),
								RidDirection::RECV });
						ELOG_DEBUG("message: added simulcast rid recv, id: %s",
								rids_.back().id.c_str());
					} else {
						ELOG_DEBUG(
								"message: invalid rid syntax: unknown direction %s length: %d",
								direction.c_str(), (int) direction.size());
					}
				} else {
				...
				ELOG_DEBUG(
						"audio shouldn't have simulcast rid! - ignoring this sdp line");
			}
		}
		...
		if (isRtpmap != std::string::npos) {
			...
			if (map_element != payload_parsed_map_.end()) {
				ELOG_DEBUG(
						"message: updating parsed ptmap to vector, PT: %u, name %s, clock %u",
				...
				ELOG_DEBUG(
						"message: adding parsed ptmap to vector, PT: %u, name %s, clock %u",
						PT, codecname.c_str(), parsed_clock);
				RtpMap new_mapping;
		...
		if (isFmtp != std::string::npos) {
			...
			for (uint part_index = 2; part_index < parts.size(); part_index++) {
				...
				std::vector<std::string> key_value = stringutil::splitOneOf(
						fmtp_line, "=", 40);
				...
				ELOG_DEBUG(
						"message: Parsing format parameter, option: %s, value: %s, PT: %u",
		...
		if (isBandwidth != std::string::npos) {
			if (mtype == VIDEO_TYPE) {
				...
				if (parts.size() >= 2) {
					...
					ELOG_DEBUG("Bandwidth for video detected %u",
							videoBandwidth);
				}
			}
		}
	...
	for (const RtpMap &internal_map : internalPayloadVector_) {
		for (const std::pair<const unsigned int, RtpMap> &parsed_pair : payload_parsed_map_) {
			...
			ELOG_DEBUG("Mapping %s/%d:%d to %s/%d:%d",
					parsed_map.encoding_name.c_str(), parsed_map.clock_rate,
					parsed_map.payload_type, internal_map.encoding_name.c_str(),
					internal_map.clock_rate, internal_map.payload_type);

			ELOG_DEBUG(
					"message: Checking feedback types, parsed: %lu, internal:%lu",
					parsed_map.feedback_types.size(),
					internal_map.feedback_types.size());

			std::vector<std::string> negotiated_feedback;
			if (!parsed_map.feedback_types.empty()
					&& !internal_map.feedback_types.empty()) {
				for (const std::string &internal_feedback_line : internal_map.feedback_types) {
					for (const std::string &parsed_feedback_line : parsed_map.feedback_types) {
						if (internal_feedback_line == parsed_feedback_line) {
							ELOG_DEBUG(
									"message: Adding feedback to codec, feedback: %s, encoding_name: %s",
									internal_feedback_line.c_str(),
									internal_map.encoding_name.c_str());
							negotiated_feedback.push_back(
									internal_feedback_line);
						}
					}
				}
			}
			...
			if (parsed_map.format_parameters.size()
				...
				for (const std::pair<std::string, std::string> &internal_parameter : internal_map.format_parameters) {
					auto found_parameter = parsed_map.format_parameters.find(
							internal_parameter.first);
					if (found_parameter != parsed_map.format_parameters.end()) {
						if (found_parameter->second
								== internal_parameter.second) {
							ELOG_DEBUG(
									"message: Adding fmtp, codec_name: %s, parameter: %s, value:%s",
									parsed_map.encoding_name.c_str(),
									found_parameter->first.c_str(),
									found_parameter->second.c_str());
							negotiated_parameters[found_parameter->first] =
									found_parameter->second;
						}
					}
				}
			}
			...
			if (internal_map.format_parameters.empty()
					|| parsed_map.format_parameters.size()
							== negotiated_parameters.size()) {
		...
		for (const RtpMap &internal_map : internalPayloadVector_) {
			if (internal_map.encoding_name == "rtx") {
				...
				if (outInPTMap[parsed_apt_pt] == internal_apt_pt) {
					...
					outInPTMap[rtx_map.payload_type] =
							internal_map.payload_type;
					inOutPTMap[internal_map.payload_type] =
							rtx_map.payload_type;
...
unsigned int SdpInfo::getAudioInternalPT(unsigned int externalPT) {
	...
	std::map<unsigned int, unsigned int>::iterator found = outInPTMap.find(
			externalPT);
...
unsigned int SdpInfo::getAudioExternalPT(unsigned int internalPT) {
	// should use separate mapping for video and audio at the very least
	// standard requires separate mappings for each media, even!
	std::map<unsigned int, unsigned int>::iterator found = inOutPTMap.find(
			internalPT);
...
bool SdpInfo::processCandidate(const std::vector<std::string> &pieces,
		MediaType mediaType, std::string line) {
	CandidateInfo cand;
	...
	ELOG_DEBUG("Candidate Info: foundation=%s, componentId=%u, netProtocol=%s, "
			...
			cand.foundation.c_str(), cand.componentId, cand.netProtocol.c_str(),
			cand.priority, cand.hostAddress.c_str(), cand.hostPort,
			cand.hostType);
