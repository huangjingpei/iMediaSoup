...
#ifndef RTC_GATWAT_SDPINFO_H_
#define RTC_GATWAT_SDPINFO_H_
...
public:
	...
	bool supportCodecByName(const std::string codecName,
			const unsigned int clockRate);
...
private:
	...
	bool processCandidate(const std::vector<std::string> &pieces,
			MediaType mediaType, std::string sdp);
	std::string stringifyCandidate(const CandidateInfo &candidate);
	void gen_random(char *s, int len);
	void maybeAddSsrcToList(uint32_t ssrc);
};

#endif  // RTC_GATWAT_SDPINFO_H_
